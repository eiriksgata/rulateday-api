package indi.eiriksgata.rulatedayapi.utils;


import indi.eiriksgata.rulatedayapi.config.ConfigureEncryptAndDecrypt;
...
public class AES {
     ...
     * @return byte[]
     */
    public static byte[] encrypt(byte[] data, byte[] key, byte[] ivValue) {
        ...
        try {
            ...
            IvParameterSpec iv = new IvParameterSpec(ivValue);//使用CBC模式，需要一个向量iv，可增加加密算法的强度
    ...
    public static String encryptWithKeyBase64(String data, String key, String iv) {
        try {
            byte[] valueByte = encrypt(
                    data.getBytes(ConfigureEncryptAndDecrypt.CHAR_ENCODING),
    ...
    public static String decryptWithKeyBase64(String data, String key, String iv) {
        try {
            byte[] originalData = Base64.decode(data.getBytes());
            byte[] valueByte = decrypt(
                    originalData, Base64.decode(key.getBytes()),
                    iv.getBytes(ConfigureEncryptAndDecrypt.CHAR_ENCODING));
            return new String(valueByte, ConfigureEncryptAndDecrypt.CHAR_ENCODING);
        } catch (UnsupportedEncodingException e) {
            throw new RuntimeException("decrypt fail!", e);
