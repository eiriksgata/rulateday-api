...
public class DiceMessageEventHandlerImpl implements DiceMessageEventHandler {
    ...
    public void onBotGroupRequest(DiceMessageDTO diceMessageDTO) {
        //检测是否是黑名单用户
        if (botControl.isBlacklist(diceMessageDTO.getWsRequestBean().getParams().getGroup_id())) {
            return;
        }
    ...
    @Override
    public void OnDiceMessage(DiceMessageDTO event) {

        if (!diceConfigMapper.selectById().getPrivate_chat()) {
    ...
    private static void groupMessageHandle(GroupMessageEvent event) {
        ...
        messageData.setMessage(messageData.getMessage().toUpperCase());
    ...
    public List<String> personalMessageEventHandler(MessageData messageData) {
